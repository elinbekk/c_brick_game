...
During operation, the input of the FSM sequentially receives entry actions, and at the output the FSM generates exit Signal. Transition from one internal state to another can occur not only from external ActionCallback, but also spontaneously.
...
- SpawnAction is the state in which the next frog is created.
...
- Collision is a state that occurs if the frog hits the water after jumping, or if the frog is outside the playing field after CollideAction logs.
...
- Start is the state in which the game waits for the player to press the ready to play button.
- Spawn is the state the game enters when you create another block and choose the next block to SpawnAction.
