...
Each game library must have a function that accepts user input. The console has eight physical buttons: start game, pause, end game, ActionCallback, and four arrows.

The `userInput` function takes as input the user `ActionCallback` and an additional parameter `hold`, which is responsible for pressing the button.
...
void userInput(UserAction_t ActionCallback, bool hold);
